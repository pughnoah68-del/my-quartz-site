...
$$ \textbf{\textit{Theorem (Alexandrov, Hausdorff)}.} \textit{Every compact metrizable space is the continuous image of the Cantor set } \mathfrak{C}.$$
When I first read this theorem, I looked up the proof in Kechris’s Descriptive Set Theory. The proof there is very short, but makes use of some helpful preliminaries. I will give a self-contained proof here:
...
We now need to define some important maps. First define $ f:X \rightarrow \mathbb{I}^{\mathbb{N}}$ where $f(x) = (d(x,x_n))_{n \in \mathbb{N} } $. This map is continuous and injective, so $ X \cong F:= im(f)$.
